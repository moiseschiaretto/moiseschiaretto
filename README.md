...
QA Sênior | Test Automation Engineer (SDET), com mais de 17 anos de experiência em Qualidade de Software — sendo os últimos 8 anos com foco forte em automação de testes seguindo a pirâmide de testes (back-end, front-end, mobile, performance, carga e segurança).
...
## 🧰 Stack principal
- **Back-end (API REST):** Playwright, Rest Assured (Java), Python (requests), Supertest+Jest, Robot Framework (RequestsLibrary) — testes de contrato em arquitetura de microsserviços (Joi, JSON Schema), múltiplos métodos HTTP, validação de status codes, Swagger, OpenAPI
